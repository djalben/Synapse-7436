...
const PricingCard = ({ tier, onSelect }: PricingCardProps) => {
  ...
  return (
    ...
    >
      {/* Badge */}
      {tier.badge && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 md:px-4 py-1 rounded-full bg-gradient-to-r from-indigo-500 to-blue-500 text-xs font-semibold text-white shadow-lg shadow-indigo-500/30 whitespace-nowrap">
          {tier.badge}
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        >
          <Icon
            className={`w-5 h-5 md:w-6 md:h-6 ${tier.highlighted ? "text-indigo-400" : "text-[#888]"}`}
          />
        </div>
        <div>
      ...
      <ul className="space-y-2 md:space-y-3 mb-6 md:mb-8 flex-1">
        {tier.features.map((feature, index) => (
          <li key={index} className="flex items-start gap-2 md:gap-3">
            ...
            >
              <Check
                className={`w-2.5 h-2.5 md:w-3 md:h-3 ${tier.highlighted ? "text-indigo-400" : "text-[#888]"}`}
              />
            </div>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 md:p-4">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl md:rounded-3xl bg-[#0a0a0a] border border-[#333] shadow-2xl">
        ...
        <div className="relative p-5 pt-10 md:p-8 md:pt-12">
          ...
          {/* Footer */}
          <div className="text-center mt-6 md:mt-10 pt-4 md:pt-6 border-t border-[#222]">
